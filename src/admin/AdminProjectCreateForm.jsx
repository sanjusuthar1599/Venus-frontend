...
export default function AdminProjectCreateForm({ onCreated }) {
  ...
  async function finish(created, message) {
  ...
  async function submitImage(e) {
    ...
    for (let i = 0; i < imageFiles.length; i++) fd.append("project_image", imageFiles[i]);
    setBusy(true);
    try {
      const created = await apiForm("/post/image/project", fd);
      await finish(created, "Project created");
    } catch (err) {
      ...
      <div className="flex gap-2 rounded-xl bg-neutral-200/60 p-1">
        <button
          ...
          className={`flex-1 rounded-lg py-2 text-sm font-semibold transition ${
            mode === "image" ? "bg-white text-neutral-900 shadow" : "text-neutral-600"
          }`}
        >
          Images
        </button>
        <button
          ...
          className={`flex-1 rounded-lg py-2 text-sm font-semibold transition ${
            mode === "video" ? "bg-white text-neutral-900 shadow" : "text-neutral-600"
          }`}
        >
          Video
        </button>
